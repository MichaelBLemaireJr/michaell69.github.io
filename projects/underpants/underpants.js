...
_.last = function(arr, num){
    ...
    let newArr = [];
    for(let i = arr.length - 1; i >= num - 1; i--){
        newArr.push(arr[i]);
    }
...
_.map = (collection, callback) => {
    ...
    _.each(collection, function(e, i, array){
        result.push(callback(e, i, array));
    });
    return result;
};
...
*/
_.contains = (array, value) => _.indexOf(array, value) >= 0;
...
_.every = (collection, callback) => {
   for(var i =  0; i < collection.length; i++){
        var curre = collection[i];
        var curr = i;
        if(callback === undefined){
            if(!curre){
                return false;
            }
    } else if(!callback(curre, curr, collection)){
            return false;
        } 
    }
    return true;
...
*/
_.reduce = (array, callback, seed) => {
    if(seed === undefined){
            seed = array[0];
            _.each(array.slice(1,array.length), function(element, index, array){
                seed = callback(seed, element, index+1, array);
            });
        } else {
...
*/
_.extend = (object1, object2, moreObjects) =>{
    _.each(object2, function(element, key, collection){
            object1[key] = object2[key]; 
        });
        _.each(moreObjects, function(element, key, collection){
            object1[key] = collection[key];
        });
